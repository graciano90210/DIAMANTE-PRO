...
def init_routes(app):
    ...
    def login():
        ...
        if user and user.check_password(password) and user.activo:
            ...
            return render_template('login.html',
     error='Usuario o contrasef1a incorrectos')
    ...
    def dashboard():
        ...
        if rol in ['dueno', 'gerente']:
            ...
            todas_las_rutas = Ruta.query.filter_by(
                activo=True).order_by(Ruta.nombre).all()
        ...
        # NOTA: Cliente.query.count() daba error si faltaban columnas GPS,
        # ahora arreglado.
        ...
        if rol == 'cobrador':
            ...
            clientes_ids_cobrador = db.session.query(
    Prestamo.cliente_id).filter_by(
        cobrador_id=usuario_id).distinct()

            total_clientes = clientes_ids_cobrador.count()

            clientes_vip = Cliente.query.filter(Cliente.id.in_(
                clientes_ids_cobrador), Cliente.es_vip).count()

            # Préstamos activos del cobrador

            prestamos_activos = Prestamo.query.filter_by(
                estado='ACTIVO', cobrador_id=usuario_id).all()
    ...
    func.sum(
        Prestamo.saldo_actual)).filter_by(
            estado='ACTIVO',
             cobrador_id=usuario_id).scalar()
    ...
    func.sum(
        Prestamo.monto_prestado)).filter_by(
            estado='ACTIVO',
             cobrador_id=usuario_id).scalar()

            capital_prestado = float(
                capital_prestado) if capital_prestado else 0
            ...
            prestamos_al_dia = sum(
                1 for p in prestamos_activos if p.cuotas_atrasadas == 0) if prestamos_activos else 0

            prestamos_atrasados = sum(
                1 for p in prestamos_activos if p.cuotas_atrasadas > 0) if prestamos_activos else 0

            prestamos_mora = sum(
                1 for p in prestamos_activos if p.cuotas_atrasadas > 3) if prestamos_activos else 0
            ...
            total_cobrado_hoy = sum(float(p.monto)
                                    for p in pagos_hoy) if pagos_hoy else 0
            ...
            # Dueño, gerente, secretaria ven todas las estadísticas (o
            # filtradas por ruta)
            ...
            if ruta_seleccionada_id:
                ...
                clientes_ids_ruta = db.session.query(
    Prestamo.cliente_id).filter_by(
         ruta_id=ruta_seleccionada_id).distinct()

                total_clientes = clientes_ids_ruta.count()

                clientes_vip = Cliente.query.filter(Cliente.id.in_(
                    clientes_ids_ruta), Cliente.es_vip).count()

                # Filtrar por ruta seleccionada

                prestamos_activos = Prestamo.query.filter_by(
                    estado='ACTIVO', ruta_id=ruta_seleccionada_id).all()
    ...
    func.sum(
        Prestamo.saldo_actual)).filter_by(
            estado='ACTIVO',
             ruta_id=ruta_seleccionada_id).scalar()
    ...
    func.sum(
        Prestamo.monto_prestado)).filter_by(
            ...
            # 1. Calcular totales de Cartera, Capital y Proyección Cobro usando
            # Préstamos Activos
            ...
            if prestamos_activos:
                ...
                for p in prestamos_activos:
                    ...
                    if p.frecuencia == 'DIARIO' and dia_semana_hoy != 6:
                        ...
                        # Simplificación: Asumimos que BISEMANAL cobra hoy si
                        # es el día correcto (esto requiere lógica adicional de
                        # fechas, pero mantenemos simple)
                    ...
                    if cobra_hoy:
                        ...
                        # Mantener suma global por compatibilidad (aunque
                        # mezcle monedas)
                        por_cobrar_hoy += float(p.valor_cuota)
        ...
        ganancia_esperada = total_cartera - \
            capital_prestado if capital_prestado > 0 else 0

        porcentaje_ganancia = (
    (ganancia_esperada /
    capital_prestado) *
     100) if capital_prestado > 0 else 0

        tasa_cobro_diaria = (total_cobrado_hoy /
                             por_cobrar_hoy * 100) if por_cobrar_hoy > 0 else 0
        ...
        # capital_total_aportado =
        # db.session.query(func.sum(AporteCapital.monto)).scalar() or 0

        #         capital_total_aportado = float(capital_total_aportado)

        #         # Total invertido en activos

        # capital_invertido_activos =
        # db.session.query(func.sum(Activo.valor_compra)).scalar() or 0

        #         capital_invertido_activos = float(capital_invertido_activos)

        #         # Capital disponible = Aportes - Activos

        # capital_disponible = capital_total_aportado -
        # capital_invertido_activos
        ...
        for i in range(7):
            ...
            if rol == 'cobrador':
                ...
                pagos_dia = Pago.query.filter(
                    func.date(Pago.fecha_pago) == fecha).all()

            total_dia = sum(float(p.monto)
                            for p in pagos_dia) if pagos_dia else 0
            ...
            labels_7_dias.append(fecha.strftime('%d/%m'))
               ...
               if rol == 'cobrador':
                    ...
                    prestamos_pagados = Prestamo.query.filter_by(
                        ...
                        estado='PAGADO', cobrador_id=usuario_id).count()

                    prestamos_cancelados = Prestamo.query.filter_by(



                        estado='CANCELADO', cobrador_id=usuario_id).count()

                    # Proyección de cobro mañana (Simplificación: misma lógica
                    # que hoy para clientes activos)
                    ...
                    prestamos_pagados = Prestamo.query.filter_by(
                        ...
                        estado='PAGADO', ruta_id=ruta_seleccionada_id).count()
                    ...
                    prestamos_cancelados = Prestamo.query.filter_by(
                        ...
                        estado='CANCELADO', ruta_id=ruta_seleccionada_id).count()
                    ...
                    prestamos_cancelados = Prestamo.query.filter_by(
                        ...
                        estado='CANCELADO').count()


                    proyeccion_manana = por_cobrar_hoy


                    # Distribución Global (Contamos clientes, no préstamos,
                    # para riesgo general)


                    riesgo_stats = db.session.query(
    Cliente.nivel_riesgo, func.count(
        Cliente.id)).group_by(
            Cliente.nivel_riesgo).all()
...
>>>>>>> df6f4440e89c0af955657c23737bb1f0ff3a397b
    ...
    def clientes_guardar():
        ...
        try:
            ...
            fecha_nacimiento = datetime.strptime(
                fecha_nac, '%Y-%m-%d').date() if fecha_nac else None
            ...
            nuevo_cliente = Cliente(
                ...
                whatsapp_codigo_pais=request.form.get(
                ...
                personas_a_cargo=to_int(
                    request.form.get('personas_a_cargo')) or 0,



                # Datos Negocio

                documento_fiscal_negocio=request.form.get(
                    'documento_fiscal_negocio'),
                ...
                antiguedad_negocio_meses=to_int(
                    request.form.get('antiguedad_negocio_meses')),

                ingresos_diarios_estimados=to_float(
                    request.form.get('ingresos_diarios_estimados')),

                gastos_mensuales_promedio=to_float(
                    request.form.get('gastos_mensuales_promedio')),
                ...
                tiempo_residencia_meses=to_int(
                    request.form.get('tiempo_residencia_meses')),

                tiene_comprobante_residencia=bool(
                    request.form.get('tiene_comprobante_residencia')),

                comprobante_a_nombre_propio=bool(
                    request.form.get('comprobante_a_nombre_propio')),
    ...
    nombre=session.get('nombre'),
     rol=session.get('rol'))
            ...
            cliente.whatsapp_codigo_pais = request.form.get(
                'whatsapp_codigo_pais', '57')
            ...
            cliente.personas_a_cargo = to_int(
                request.form.get('personas_a_cargo')) or 0

            # Datos Negocio / Fiscal

            cliente.documento_fiscal_negocio = request.form.get(
                'documento_fiscal_negocio')
            ...
            cliente.antiguedad_negocio_meses = to_int(
                request.form.get('antiguedad_negocio_meses'))

            cliente.ingresos_diarios_estimados = to_float(
                request.form.get('ingresos_diarios_estimados'))

            cliente.gastos_mensuales_promedio = to_float(
                request.form.get('gastos_mensuales_promedio'))
            ...
            cliente.tiempo_residencia_meses = to_int(
                request.form.get('tiempo_residencia_meses'))

            cliente.tiene_comprobante_residencia = bool(
                request.form.get('tiene_comprobante_residencia'))

            cliente.comprobante_a_nombre_propio = bool(
                request.form.get('comprobante_a_nombre_propio'))
...
=======
    ...
    cobrador_id=usuario_id).order_by(
...
>>>>>>> df6f4440e89c0af955657c23737bb1f0ff3a397b
            ...
            total_cartera = db.session.query(func.sum(Prestamo.saldo_actual)).filter(
                ...
                Prestamo.cobrador_id == usuario_id
    func.sum(
        Prestamo.monto_a_pagar -
        Prestamo.monto_prestado) ).filter(
            Prestamo.estado == 'ACTIVO',
             Prestamo.cobrador_id == usuario_id).scalar() or 0
...
>>>>>>> df6f4440e89c0af955657c23737bb1f0ff3a397b
    ...
    def prestamos_nuevo():
        ...
        # Filtrar clientes: mostrar solo los de la ruta actual + nuevos (sin
        # préstamos)
    ...
    r[0] for r in db.session.query(
        Prestamo.cliente_id).filter_by(
            ruta_id=ruta_id).distinct().all()]

            # 2. Clientes SIN prestamos (Nuevos)

            clientes_nuevos_ids = [r[0] for r in db.session.query(
                Cliente.id).outerjoin(Prestamo).filter(Prestamo.id is None).all()]

            ids_validos = list(set(clientes_ruta_ids + clientes_nuevos_ids))

            clientes = Cliente.query.filter(Cliente.id.in_(
                ids_validos)).order_by(Cliente.nombre).all()
        ...
        # Cobradores pueden ser: supervisor, cobrador, o admin (por
        # compatibilidad)

        cobradores = Usuario.query.filter(Usuario.rol.in_(
            ['admin', 'dueno', 'supervisor', 'cobrador'])).all()
    ...
    def prestamos_guardar():
        ...
        try:
            ...
            if prestamo_activo:
                ...
                cobradores = Usuario.query.filter_by(
                    rol='cobrador', activo=True).all()

                return render_template(
    'prestamos_nuevo.html',
    error=f'❌ Este cliente ya tiene un préstamo activo (#ID: {
        prestamo_activo.id}). No puede tener dos préstamos simultáneos.',
        clientes=clientes,
        cobradores=cobradores,
        nombre=session.get('nombre'),
         rol=session.get('rol'))
            ...
            if frecuencia == 'DIARIO':
                ...
                # Considerar solo días laborables (lun-sáb) -> 6 días por
                # semana
                ...
                # Considerar solo días laborables (lun-vie) -> 5 días por
                # semana
            ...
            if not ruta_id:
                ...
                ruta = Ruta.query.filter_by(
                    cobrador_id=cobrador_id_form).first()
                ...
                if ruta:
                    ...
                    # Fallback: Usar la primera ruta disponible para evitar
        ...
        'prestamo_exito',
         prestamo_id=nuevo_prestamo.id))
            ...
            cobradores = Usuario.query.filter(
                Usuario.rol.in_(['admin', 'cobrador'])).all()
    ...
    cobradores=cobradores,
    fecha_hoy=datetime.now().strftime('%Y-%m-%d'),
    error=f'Error al crear préstamo: {
        str(e)}',
        nombre=session.get('nombre'),
         rol=session.get('rol'))
    ...
    def prestamo_detalle(prestamo_id):
        ...
        pagos = Pago.query.filter_by(prestamo_id=prestamo_id).order_by(
            Pago.fecha_pago.desc()).all()
    ...
    def prestamo_comprobante_imagen(prestamo_id):
        ...
        # Crear imagen en memoria (1080x1350 - Formato Instagram Portrait más
        # compacto)
        ...
        try:
            ...
            font_title = ImageFont.truetype(
                "arialbd.ttf", 70)      # Título principal

            font_subtitle = ImageFont.truetype(
                "arial.ttf", 35)     # Subtítulos

            font_section = ImageFont.truetype(
                "arialbd.ttf", 35)    # Encabezados de sección

            font_label = ImageFont.truetype(
                "arial.ttf", 30)        # Etiquetas pequeñas

            font_value = ImageFont.truetype("arialbd.ttf", 38)      # Valores

            font_money = ImageFont.truetype(
                "arialbd.ttf", 60)      # Cifras grandes

            font_footer = ImageFont.truetype(
                "arial.ttf", 28)       # Pie de página
        ...
        draw.text((width //2, 100), "💎 DIAMANTE PRO",
                  fill=c_white, font=font_title, anchor='mm')

        draw.text((width //2, 180), "COMPROBANTE OFICIAL DE CRÉDITO",
                  fill='#e2e8f0', font=font_subtitle, anchor='mm')
        ...
        draw.rectangle([tag_x, tag_y, tag_x + tag_w, tag_y + \
                       tag_h], fill=c_accent)  # Fondo etiqueta azul

        draw.text((width //2, tag_y + 30),
                  f"CRÉDITO #{prestamo.id}  •  {prestamo.fecha_inicio.strftime('%d/%m/%Y')}", fill=c_white, font=font_section, anchor='mm')
        ...
        def draw_card(y_pos, height):
            ...
            draw.rectangle([margin_x, y_pos, width-margin_x, y_pos + \
                           height], fill=c_white, outline='#cbd5e1', width=1)
        ...
        draw.text((margin_x + 40, current_y + 50),
                  "👤 DATOS DEL CLIENTE", fill=c_accent, font=font_section)

        # Nombre Cliente

        draw.text((margin_x + 40, current_y + 110),
                  cliente.nombre.upper(), fill=c_primary, font=font_money)
    ...
    f"Documento: {
        ...
        draw.line([margin_x + 40, current_y + 200, width -
                  margin_x - 40, current_y + 200], fill='#e2e8f0', width=2)

        # Total a Pagar destacado

        draw.text((margin_x + 40, current_y + 230),
                  "MONTO TOTAL A PAGAR", fill=c_muted, font=font_label)
    ...
    font=font_money,
     anchor='ra')
        ...
        draw.text((margin_x + 40, current_y + 50),
                  "📊 DETALLES DEL PRÉSTAMO", fill=c_accent, font=font_section)
        ...
        draw.text((col1_x, row_start), "Monto Prestado:",
                  fill=c_muted, font=font_label)

        draw.text((col1_x,
    row_start + 40),
    f"${prestamo.monto_prestado:,.0f}",
    fill=c_text,
     font=font_value)

        draw.text((col1_x, row_start + row_space),
                  "Tasa Interés:", fill=c_muted, font=font_label)

        draw.text((col1_x, row_start + row_space + 40),
                  f"{prestamo.tasa_interes:.0f}%", fill=c_text, font=font_value)

        draw.text((col1_x, row_start + row_space * 2),
                  "Estado:", fill=c_muted, font=font_label)

        estado_color = c_success if prestamo.estado == 'ACTIVO' else c_muted

        draw.text((col1_x, row_start + row_space * 2 + 40),
                  f"✅ {prestamo.estado}", fill=estado_color, font=font_value)
        ...
        draw.text((col2_x,
    row_start + 40),
    f"${prestamo.valor_cuota:,.0f} ({prestamo.frecuencia})",
    fill=c_text,
     font=font_value)

        draw.text((col2_x, row_start + row_space),
                  "Nº Cuotas:", fill=c_muted, font=font_label)

        draw.text((col2_x, row_start + row_space + 40),
                  f"{prestamo.numero_cuotas}", fill=c_text, font=font_value)

        draw.text((col2_x, row_start + row_space * 2),
                  "Vence:", fill=c_muted, font=font_label)

        draw.text((col2_x,
    row_start + row_space * 2 + 40),
    f"{prestamo.fecha_fin_estimada.strftime('%d/%m/%Y')}",
    fill=c_text,
     font=font_value)
        ...
        draw.line([margin_x, current_y, width - margin_x,
                  current_y], fill='#cbd5e1', width=2)

        current_y += 30

        draw.text((width //2, current_y),
                  f"Atendido por: {cobrador.nombre}", fill=c_muted, font=font_footer, anchor='mm')

        draw.text((width //2, current_y + 40),
                  f"Fecha de emisión: {datetime.now().strftime('%d/%m/%Y %H:%M')}", fill=c_muted, font=font_footer, anchor='mm')

        # Mensaje final

        draw.rectangle([0, height -100, width, height], fill='#f1f5f9')

        draw.text((width //2, height-50), "¡Gracias por su confianza!",
                  fill=c_accent, font=font_section, anchor='mm')
    ...
    mimetype='image/png',
    as_attachment=True,
    download_name=f'Comprobante_Credito_{
        prestamo.id}_{
    ...
    def cobro_lista():
        ...
        creditos_atrasados = sum(
            1 for p in prestamos if 0 < p.cuotas_atrasadas <= 3)
    ...
    def cobro_guardar():
        ...
        try:
            ...
            # Refrescar el objeto desde la base de datos para obtener el saldo
            # más actual
            ...
            # Verificar si ya existe un pago con el mismo monto hoy (solo si no
            # está forzando)
    ...
    prestamo=prestamo,
    error=f'⚠️ Ya se registró un pago de {
        prestamo.moneda} {
            monto:,.0f} para este cliente hoy a las {
                pago_duplicado.fecha_pago.strftime("%H:%M")}. ¿Está seguro de registrar otro pago con el mismo valor?',
                nombre=session.get('nombre'),
                 rol=session.get('rol'))
            ...
            if tipo_pago == 'COMPLETO':
                ...
                cuotas_pagadas = int(request.form.get(
            ...
            if prestamo.cuotas_atrasadas > 0:
                ...
                prestamo.cuotas_atrasadas = max(
                    0, prestamo.cuotas_atrasadas - cuotas_pagadas)
...
Gracias por su pago!"""
        ...
        whatsapp_url = f"https://wa.me/{
    cliente.whatsapp_completo}?text={mensaje_encoded}"
    ...
    def cobro_recibo_imagen(pago_id):
        ...
        try:
            ...
            font_money_main = ImageFont.truetype(
    "arialbd.ttf", 80)  # Cifra central enorme
        ...
        draw.text((width //2, 180), "RECIBO DE PAGO OFICIAL",
                  fill='#ecfdf5', font=font_subtitle, anchor='mm')
        ...
        draw.rectangle([tag_x, tag_y, tag_x + tag_w, tag_y + \
                       tag_h], fill='#047857')  # Verde más oscuro

        draw.text((width //2, tag_y + 30),
                  f"RECIBO #{pago.id}  •  {pago.fecha_pago.strftime('%d/%m/%Y %H:%M')}", fill=c_white, font=font_section, anchor='mm')
        ...
        draw.text((margin_x + 40, current_y + 50),
                  "👤 DATOS DEL CLIENTE", fill=c_success, font=font_section)
    ...
    f"Documento: {
        ...
        draw.rectangle([margin_x, current_y, width-margin_x, current_y + \
                       card2_h], fill=c_success_bg, outline='#10b981', width=2)

        draw.text((width //2, current_y + 60), "MONTO RECIBIDO",
                  fill='#047857', font=font_section, anchor='mm')

        draw.text((width //2, current_y + 140),
                  f"{prestamo.moneda} ${pago.monto:,.0f}", fill='#065f46', font=font_money_main, anchor='mm')
        ...
        draw.text((margin_x + 40, current_y + 50),
                  "📉 ESTADO DE CUENTA", fill=c_primary, font=font_section)
        ...
        draw.text((col1_x, row_start), "Saldo Anterior:",
                  fill=c_muted, font=font_label)

        draw.text((col1_x, row_start + 40),
                  f"${pago.saldo_anterior:,.0f}", fill=c_text, font=font_value)

        # Saldo Nuevo (Destacado)

        draw.text((col2_x, row_start), "Nuevo Saldo:",
                  fill=c_muted, font=font_label)

        draw.text((col2_x, row_start + 40),
                  f"${pago.saldo_nuevo:,.0f}", fill=c_accent, font=font_money)
        ...
        if pago.observaciones:
            ...
            draw.text((margin_x, current_y), "📝 Observaciones:",
                      fill=c_muted, font=font_label)

            obs_text = pago.observaciones[:80] + "..." if len(
                pago.observaciones) > 80 else pago.observaciones

            draw.text((margin_x + 230, current_y), obs_text,
                      fill=c_text, font=font_label)
        ...
        draw.text((width //2, current_y),
                  f"Recibido por: {session.get('nombre')}", fill=c_muted, font=font_footer, anchor='mm')

        # Mensaje final

        draw.rectangle([0, height -80, width, height], fill='#f1f5f9')

        draw.text((width //2, height-40), "¡Gracias por su pago a tiempo!",
                  fill=c_success, font=font_section, anchor='mm')
    ...
    def reportes():
        ...
        if request.args.get('fecha_inicio'):
            ...
            fecha_inicio = datetime.strptime(
                request.args.get('fecha_inicio'), '%Y-%m-%d')

        if request.args.get('fecha_fin'):

            fecha_fin = datetime.strptime(
                request.args.get('fecha_fin'), '%Y-%m-%d')
        ...
        if rol == 'cobrador':
            ...
            total_prestamos = Prestamo.query.filter_by(
                cobrador_id=usuario_id).count()

            prestamos_activos = Prestamo.query.filter_by(
                estado='ACTIVO', cobrador_id=usuario_id).count()
        ...
        if rol == 'cobrador':
            ...
            total_prestado_periodo = sum(
                p.monto_prestado for p in prestamos_periodo)
            ...
            cartera_actual = db.session.query(
    func.sum(
        Prestamo.saldo_actual)).filter_by(
            estado='ACTIVO',
             cobrador_id=usuario_id).scalar()
            ...
            capital_circulacion = db.session.query(
    func.sum(
        Prestamo.monto_prestado)).filter_by(
            estado='ACTIVO',
             cobrador_id=usuario_id).scalar()

            capital_circulacion = float(
                capital_circulacion) if capital_circulacion else 0
    ...
    Prestamo.frecuencia,
    func.count(
        Prestamo.id).label('cantidad') ).filter_by(
            estado='ACTIVO',
            cobrador_id=usuario_id).group_by(
                Prestamo.frecuencia).all()
    ...
    def usuarios_lista():
        ...
        for usuario in usuarios:
            ...
            total_cobrado = db.session.query(func.sum(Pago.monto)).filter_by(
                cobrador_id=usuario.id).scalar()
    ...
    usuario=usuario,
    error=f'Error al actualizar usuario: {
        str(e)}',
        nombre=session.get('nombre'),
         rol=session.get('rol'))
    ...
    def sociedades_lista():
        ...
        sociedades = Sociedad.query.order_by(
            Sociedad.fecha_creacion.desc()).all()
        ...
        for sociedad in sociedades:
            ...
            num_rutas = Ruta.query.filter_by(
                sociedad_id=sociedad.id, activo=True).count()
    ...
    nombre=session.get('nombre'),
     rol=session.get('rol'))
            ...
            sociedad.nombre_socio_2 = request.form.get(
                'nombre_socio_2') or None

            sociedad.telefono_socio_2 = request.form.get(
                'telefono_socio_2') or None

            sociedad.porcentaje_socio_2 = p2

            # Socio 3

            sociedad.nombre_socio_3 = request.form.get(
                'nombre_socio_3') or None

            sociedad.telefono_socio_3 = request.form.get(
                'telefono_socio_3') or None
    ...
    'sociedades_editar.html',
    sociedad=sociedad,
    error=f'Error al actualizar sociedad: {
        str(e)}',
        nombre=session.get('nombre'),
         rol=session.get('rol'))
    ...
    def capital_lista():
        ...
        aportes = AporteCapital.query.order_by(
            AporteCapital.fecha_aporte.desc()).all()
    ...
    def capital_guardar():
        ...
        try:
            ...
            ingreso_caja = Transaccion(
                ...
                # Entra a la caja del usuario actual (Dueño)
    ...
    def activos_lista():
        ...
        total_valor = db.session.query(
            func.sum(Activo.valor_compra)).scalar() or 0
    ...
    def rutas_lista():
        ...
        for ruta in rutas:
            ...
            num_prestamos = Prestamo.query.filter_by(
                ruta_id=ruta.id, estado='ACTIVO').count()
    ...
    def rutas_guardar():
        ...
        try:
            ...
            nueva_ruta = Ruta(
                ...
                sociedad_id=int(
                    sociedad_id) if sociedad_id and sociedad_id != '' else None,
    ...
    'supervisor',
     'dueno']:
        ...
        cobradores = Usuario.query.filter(
            Usuario.rol.in_(['cobrador', 'supervisor'])).all()
    ...
    'supervisor',
     'dueno']:
        ...
        # Si se especifica cobrador_id en la URL, usar ese; sino usar el
        # usuario actual
        ...
        # Préstamos activos del cobrador que NO pagaron hoy y tienen frecuencia
        # DIARIO o BISEMANAL
        ...
        title = Paragraph(
            f"<b>{usuario.nombre.upper()} - INFORME</b>", styles['Title'])
        ...
        fecha_text = Paragraph(
            f"Fecha {fecha.strftime('%d/%m/%Y')}", styles['Normal'])
        ...
        cuadre_data = [
            ...
            [f'${total_abonos:,.2f}', f'${total_desembolsos:,.2f}',
                f'${total_caja:,.2f}']
        ...
        elements.append(
            Paragraph("<b>GASTOS Y MOVIMIENTOS</b>", styles['Heading2']))
        ...
        if gastos:
            ...
            for gasto in gastos:
                ...
                gastos_data.append([
                    ...
                    gasto.descripcion[:40] + '...' if gasto.descripcion and len(
                        gasto.descripcion) > 40 else (gasto.descripcion or ''),

                    f"${gasto.monto:,.2f}"

                ])

            gastos_table = Table(gastos_data, colWidths=[
            ...
            elements.append(
                Paragraph("No hay gastos registrados", styles['Normal']))
        ...
        if prestamos_hoy:
            ...
            creditos_table = Table(creditos_data, colWidths=[
                                   3 *inch, 1.5*inch, 1.5*inch])
        ...
        if clientes_sin_pago:
            ...
            elements.append(
                Paragraph("<b>CLIENTES SIN PAGO</b>", styles['Heading2']))

            sin_pago_data = [['N°', 'CLIENTE', 'CELULAR', 'VALOR']]

            # Máximo 10
            for idx, cliente in enumerate(clientes_sin_pago[:10], 1):
            ...
            sin_pago_table = Table(sin_pago_data, colWidths=[
        ...
        response.headers[ 'Content-Disposition'] = f'attachment; filename=cuadre_ruta_{
    fecha.strftime("%Y%m%d")}.pdf'
    ...
    def caja_inicio():
        ...
        total_traslados_recibidos = sum(
            t.monto for t in traslados_recibidos_hoy)
        ...
        if rol == 'cobrador':
            ...
            gastos_hoy = Transaccion.query.filter(
                func.date(Transaccion.fecha) == hoy).all()

        total_gastos_hoy = sum(g.monto for g in gastos_hoy)

        # Balance del día (cobros + traslados recibidos - gastos - traslados
        # enviados)
        ...
        if rol == 'cobrador':
            ...
            gastos_mes = Transaccion.query.filter(
                Transaccion.fecha >= inicio_mes).all()
            ...
            file.filename}")
                    ...
                    if boto3 and s3_bucket:
                        ...
                        try:
                            ...
                            s3.upload_fileobj(
    file, s3_bucket, filename, ExtraArgs={
         'ContentType': file.content_type, 'ACL': 'public-read'} )
                            ...
                            # O path-style (más seguro por default):
                            ...
                            print(
                                f"⚠️ Error subiendo a S3: {e_s3}. Usando almacenamiento local temporal.")

                            # Fallback a local si falla S3

                            upload_folder = os.path.join(
                                app.root_path, 'static', 'uploads', 'recibos')
    ...
    url_for(
        'caja_gastos',
         mensaje='Gasto registrado exitosamente'))
            ...
            return render_template(
    'caja_gastos_nuevo.html',
    fecha_hoy=datetime.now().strftime('%Y-%m-%d'),
    error=f'Error al guardar: {
        str(e)}',
        nombre=session.get('nombre'),
         rol=session.get('rol'))
    ...
    def caja_cuadre():
        ...
        if rol == 'cobrador':
            ...
            gastos = Transaccion.query.filter(
                func.date(Transaccion.fecha) == fecha).all()
        ...
        try:
            ...
            if tipo_traslado == 'general_a_ruta':
                ...
                transaccion = Transaccion(
                    ...
                    # Quien hace el traslado (admin/supervisor)
                ...
                cobrador_origen_id = int(
                    request.form.get('cobrador_origen_id'))

                cobrador_destino_id = int(
                    request.form.get('cobrador_destino_id'))
                ...
                if cobrador_origen_id == cobrador_destino_id:
                    ...
                    raise ValueError(
                        'El cobrador origen y destino no pueden ser el mismo')
    ...
    url_for(
        'traslados_exito',
         traslado_id=transaccion.id))
            ...
            return render_template(
    'traslados_nuevo.html',
    cobradores=cobradores,
    fecha_hoy=datetime.now().strftime('%Y-%m-%d'),
    error=f'Error al registrar traslado: {
        str(e)}',
        nombre=session.get('nombre'),
         rol=session.get('rol'))
