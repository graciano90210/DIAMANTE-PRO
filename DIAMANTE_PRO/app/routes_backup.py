...
# --- imports ---
from flask import render_template, request, redirect, url_for, session, flash, make_response, send_file, Blueprint
...
# Definición del Blueprint debe ir antes de cualquier uso de @main.route
...
# Ruta principal (home) redirige a login si no hay sesión
@main.route('/')
def home():
    if 'usuario_id' not in session:
...
# ...aquí irían las funciones de autenticación y clientes hasta la línea 500...
...
def dashboard():
    # Variables adicionales para evitar errores en dashboard.html
    capital_total_aportado = db.session.query(func.sum(AporteCapital.monto)).scalar() or 0
    notificaciones = []
    mensajes_sin_leer = 0
    # Calcular total_clientes si no existe
    total_clientes = Cliente.query.count() if 'total_clientes' not in locals() else total_clientes
    # Definir clientes_vip temporalmente en 0 para evitar error en dashboard.html
    clientes_vip = 0
    if 'usuario_id' not in session:
        return redirect(url_for('main.login'))
    usuario_id = session.get('usuario_id')
    ...
    # Cargar todas las rutas para el filtro del dashboard
    todas_las_rutas = Ruta.query.order_by(Ruta.nombre).all()
    
    # Cargar la ruta seleccionada si existe
    ...
    # Distribución de préstamos por estado y estadísticas principales
    if rol == 'cobrador':
        prestamos_pagados = Prestamo.query.filter_by(
            estado='PAGADO', cobrador_id=usuario_id
        ).count()
        prestamos_cancelados = Prestamo.query.filter_by(
            estado='CANCELADO', cobrador_id=usuario_id
        ).count()
        proyeccion_manana = 0  # Se calcula más abajo
        ...
        prestamos_activos = Prestamo.query.filter_by(
            estado='ACTIVO', cobrador_id=usuario_id
        ).all()
    elif ruta_seleccionada_id:
        prestamos_pagados = Prestamo.query.filter_by(
            estado='PAGADO', ruta_id=ruta_seleccionada_id
        ).count()
        prestamos_cancelados = Prestamo.query.filter_by(
            estado='CANCELADO', ruta_id=ruta_seleccionada_id
        ).count()
        proyeccion_manana = 0  # Se calcula más abajo
        riesgo_stats = db.session.query(
            Cliente.nivel_riesgo, func.count(Cliente.id)
        ...
        prestamos_activos = Prestamo.query.filter_by(
            estado='ACTIVO', ruta_id=ruta_seleccionada_id
        ).all()
    else:
        prestamos_pagados = Prestamo.query.filter_by(
            estado='PAGADO'
    ...
    prestamos_al_dia = sum(
        1 for p in prestamos_activos if p.cuotas_atrasadas == 0
    ) if prestamos_activos else 0
    prestamos_atrasados = sum(
        1 for p in prestamos_activos if p.cuotas_atrasadas > 0
    ) if prestamos_activos else 0
    prestamos_mora = sum(
        1 for p in prestamos_activos if p.cuotas_atrasadas > 3
    ) if prestamos_activos else 0
    total_cobrado_hoy = sum(
        float(p.monto) for p in pagos_hoy
    ) if pagos_hoy else 0
    num_pagos_hoy = len(pagos_hoy)
    ganancia_esperada = total_cartera - capital_prestado if capital_prestado > 0 else 0
    porcentaje_ganancia = (
        (ganancia_esperada / capital_prestado) * 100
    ) if capital_prestado > 0 else 0
    tasa_cobro_diaria = (
        total_cobrado_hoy / por_cobrar_hoy * 100
    ) if por_cobrar_hoy > 0 else 0
    riesgo_labels = []
    riesgo_data = []
...
@main.route('/clientes/guardar', methods=['POST'])
...
@main.route('/clientes/editar/<int:cliente_id>')
...
@main.route('/clientes/actualizar/<int:cliente_id>', methods=['POST'])
...
def prestamos_lista():
    ...
    usuario_id = session.get('usuario_id') if 'usuario_id' in session else None
    total_prestado = db.session.query(func.sum(Prestamo.monto_prestado)).filter(
        Prestamo.estado == 'ACTIVO', 
        Prestamo.cobrador_id == usuario_id if usuario_id is not None else 0
    ).scalar() or 0
    ...
    cobradores = cobradores if 'cobradores' in locals() else []
    cliente_id_seleccionado = cliente_id_seleccionado if 'cliente_id_seleccionado' in locals() else None
...
@main.route('/prestamos/guardar', methods=['POST'])
...
@main.route('/prestamos/ver/<int:prestamo_id>')
...
@main.route('/prestamos/exito/<int:prestamo_id>')
...
@main.route('/prestamos/comprobante-imagen/<int:prestamo_id>')
...
def cobro_lista():
    ...
    prestamos = prestamos if 'prestamos' in locals() else []
...
@main.route('/cobro/registrar/<int:prestamo_id>')
...
@main.route('/cobro/guardar', methods=['POST'])
def cobro_guardar():
...
@main.route('/cobro/exito/<int:pago_id>')
...
@main.route('/cobro/recibo-imagen/<int:pago_id>')
...
@main.route('/usuarios/eliminar/<int:usuario_id>')
...
@main.route('/sociedades/guardar', methods=['POST'])
...
nombre=session.get('nombre'),
 rol=session.get('rol'))
        ...
        return redirect(url_for('main.sociedades_lista'))
...
@main.route('/sociedades/editar/<int:sociedad_id>')
...
@main.route('/sociedades/actualizar/<int:sociedad_id>', methods=['POST'])
...
@main.route('/capital/guardar', methods=['POST'])
def capital_guardar():
    ...
    try:
        ...
        return redirect(url_for('main.capital_lista'))
...
@main.route('/activos/guardar', methods=['POST'])
def activos_guardar():
...
notas=notas,
 registrado_por_id=session.get('usuario_id') )
        ...
        return redirect(url_for('main.activos_lista'))
...
def rutas_guardar():
    ...
    try:
        ...
        return redirect(url_for('main.rutas_lista'))
...
@main.route('/rutas/editar/<int:ruta_id>')
...
@main.route('/rutas/actualizar/<int:ruta_id>', methods=['POST'])
...
@main.route('/reporte/seleccionar-cobrador')
...
@main.route('/caja/gastos/guardar', methods=['POST'])
...
@main.route('/traslados/guardar', methods=['POST'])
...
@main.route('/traslados/exito/<int:traslado_id>')
